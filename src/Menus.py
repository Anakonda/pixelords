...
class Menu:
	def __init__(self, initialization):
		...
		while self.running:
			...
			self.initialization.clock.tick(100)
	...
	def draw(self):
		self.initialization.screen.fill((0,0,0))
	...
	def gotoMenu(self, menu):
		self.quit()
		self.initialization.menu = menu(self.initialization)
	...
	class Button:
		...
		def draw(self):
			self.menu.initialization.screen.fill((0,128,0),((self.x,self.y),(self.sizex,self.sizey)))

			text = self.menu.initialization.text2.render(self.text, True, (255,255,255))
			self.menu.initialization.screen.blit(text, (((2*self.x+self.sizex)-text.get_width())/2,((2*self.y+self.sizey)-text.get_height())/2))
...
class MainMenu(Menu):
	...
	def startGame(self):
		game = Game.Game(self.initialization)
